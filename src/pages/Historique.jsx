  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
        {activites.map(a => (
          <button key={a.id} onClick={() => setActId(a.id)} style={{ padding: '6px 12px', borderRadius: 7, border: '1px solid ' + (actId === a.id ? a.couleur : '#E2E8F0'), background: actId === a.id ? a.couleur + '12' : 'transparent', color: actId === a.id ? a.couleur : '#475569', fontSize: 13, fontWeight: actId === a.id ? 600 : 500, cursor: 'pointer', fontFamily: 'inherit' }}>{a.icone} {a.nom}</button>
        ))}
      </div>
      <div style={S.card}>
        ...
        {tl.length > 0 && (
          <div style={{ fontSize: 12, color: '#64748B', marginBottom: 8 }}>
            {savedTl.length} saisie(s){missingTl.length > 0 && <span style={{ color: '#E11D48', fontWeight: 600 }}> · {missingTl.length} manquante(s)</span>} · du {fmtS(firstDate)} au {fmtS(lastDate)}
          </div>
        )}
        {missingTl.length > 0 && (
          <div style={{ padding: '6px 10px', background: '#E11D4808', border: '1px solid #E11D4833', borderRadius: 6, marginBottom: 10, fontSize: 12, color: '#E11D48' }}>
            ⚠ {missingTl.length} {act?.nom?.toLowerCase()} non saisie(s) : {missingTl.slice(-3).map(t => fmtS(t.date)).join(', ')}{missingTl.length > 3 ? '...' : ''}
          </div>
        )}
        <div style={{ display: 'flex', gap: 12, fontSize: 11, color: '#64748B', marginBottom: 8, flexWrap: 'wrap' }}>
          <span><span style={{ display: 'inline-block', width: 10, height: 10, background: '#059669', borderRadius: 2, marginRight: 3, verticalAlign: 'middle' }} /> Saisie</span>
          <span><span style={{ display: 'inline-block', width: 10, height: 10, background: 'repeating-linear-gradient(45deg, #BA7517, #BA7517 3px, transparent 3px, transparent 6px)', border: '1px solid #BA7517', borderRadius: 2, marginRight: 3, verticalAlign: 'middle' }} /> Annulée</span>
          <span><span style={{ display: 'inline-block', width: 10, height: 10, background: 'transparent', border: '2px dashed #E11D48', borderRadius: 2, marginRight: 3, verticalAlign: 'middle' }} /> Non saisie</span>
        </div>
        {tl.length === 0 ? <div style={{ padding: 20, textAlign: 'center', color: '#64748B', fontSize: 13 }}>Aucune donnée</div>
          : <div>
                ...
                {tl.map(t => {
                  ...
                  if (t.status === 'missing') {
                    return (
                      <div key={t.date} title={"Non saisie — " + t.date} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '1 0 28px', maxWidth: 36 }}>
                        <div style={{ fontSize: 10, color: '#E11D48', marginBottom: 2, fontWeight: 700 }}>?</div>
                        <div style={{ width: '70%', background: 'transparent', border: '2px dashed #E11D48', borderRadius: '3px 3px 0 0', height: '100px', boxSizing: 'border-box' }} />
                        <div style={{ fontSize: 7, color: '#E11D48', marginTop: 2, transform: 'rotate(-45deg)', transformOrigin: 'center', whiteSpace: 'nowrap', fontWeight: 600 }}>{label}</div>
                      </div>
                    )
                  }
                  if (t.status === 'cancelled') {
                    return (
                      <div key={t.date} title={"Annulée — " + (t.motif || '')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '1 0 28px', maxWidth: 36 }}>
                        <div style={{ fontSize: 10, color: '#BA7517', marginBottom: 2, fontWeight: 700 }}>—</div>
                        <div style={{ width: '70%', background: 'repeating-linear-gradient(45deg, #BA751733, #BA751733 3px, transparent 3px, transparent 6px)', border: '1px solid #BA7517', borderRadius: '3px 3px 0 0', height: '80px', boxSizing: 'border-box' }} />
                        <div style={{ fontSize: 7, color: '#BA7517', marginTop: 2, transform: 'rotate(-45deg)', transformOrigin: 'center', whiteSpace: 'nowrap' }}>{label}</div>
                      </div>
                    )
                  }
                  ...
                  return (
                    <div key={t.date} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '1 0 28px', maxWidth: 36 }}>
                      <div style={{ fontSize: 10, color: '#475569', marginBottom: 2, fontWeight: 600 }}>{pct}%</div>
                      <div style={{ width: '70%', background: pct >= 80 ? '#059669' : pct >= 50 ? '#BA7517' : '#E11D48', borderRadius: '3px 3px 0 0', height: Math.max(6, pct * 1.2) + 'px' }} />
                      <div style={{ fontSize: 7, color: '#64748B', marginTop: 2, transform: 'rotate(-45deg)', transformOrigin: 'center', whiteSpace: 'nowrap' }}>{label}</div>
                    </div>
                  )
                })}
              </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 12 }}>
              ...
              <tbody>{tl.slice().reverse().slice(0, 20).map(t => {
                if (t.status === 'missing') return <tr key={t.date}><td style={S.td}>{fmt(t.date)}</td><td style={{ ...S.td, color: '#E11D48', fontWeight: 600 }}>Non saisie</td><td style={S.td}>—</td><td style={S.td}>—</td><td style={S.td}>—</td></tr>
                if (t.status === 'cancelled') return <tr key={t.date}><td style={S.td}>{fmt(t.date)}</td><td style={{ ...S.td, color: '#BA7517', fontWeight: 600 }}>Annulée{t.motif ? ' — ' + t.motif : ''}</td><td style={S.td}>—</td><td style={S.td}>—</td><td style={S.td}>—</td></tr>
                const pct = t.el > 0 ? Math.round(t.pr / t.el * 100) : 0
                return <tr key={t.date}><td style={S.td}>{fmt(t.date)}</td><td style={{ ...S.td, color: '#059669' }}>Saisie</td><td style={{ ...S.td, fontWeight: 600 }}>{t.pr}</td><td style={S.td}>{t.el}</td><td style={S.td}><span style={{ fontWeight: 600, color: pct >= 80 ? '#059669' : pct >= 50 ? '#BA7517' : '#E11D48' }}>{pct}%</span></td></tr>
              })}</tbody>
            </table>
          </div>}
      </div>
