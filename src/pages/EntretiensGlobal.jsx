...
export default function EntretiensPage({ entretiens, membres, actifs, refs, openFiche, ajouterEnt, showToast }) {
  ...
  // #14 - Entretiens planifiés en retard
  const planifiesRetard = all.filter(e => e.statut === 'Planifié' && e.date_entretien && new Date(e.date_entretien) < new Date(today()))
  ...
  const handleSave = async () => {
    ...
    await ajouterEnt({ membre_id: fd.membre_id, date_entretien: fd.date_entretien || today(), avec_qui: fd.avec_qui || null, sujet_id: fd.sujet_id || null, sujet_libre: fd.sujet_libre || null, statut: fd.statut || 'Réalisé', commentaires: fd.commentaires || '' })
    setModal(null); setFd({})
  }
    ...
    <div>
      ...
      <div style={S.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <button onClick={() => { setFd({ statut: 'Réalisé', date_entretien: today() }); setModal('new') }} style={{ ...S.btn('#3060d0', false), display: 'flex', alignItems: 'center', gap: 4 }}><Plus size={13} /> Entretien</button>
        </div>
