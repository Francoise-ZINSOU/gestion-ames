...
export default function AlertesPage({ alertes, membres, openFiche, entretiens, refs }) {
  ...
  return (
    <div style={S.card}>
      <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Âmes nécessitant attention ({filtered.length})</div>
      <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 10 }}>Score = absences (3pts) + sans entretien (2pts) + défis ouverts (1pt). Les membres avec un entretien planifié dans les 30 prochains jours sont masqués.</div>
      {filtered.length === 0 ? <div style={{ padding: 12, textAlign: 'center', color: '#1a9c60', fontSize: 12 }}>✓ Tout est à jour !</div>
        : filtered.map(a => {
