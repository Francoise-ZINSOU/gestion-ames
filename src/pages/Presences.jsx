  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <select className="mob-only" value={actId} onChange={e => setActId(e.target.value)} style={{ flex: 1, padding: '8px 10px', borderRadius: 7, border: '1px solid ' + (act?.couleur || '#c8cfe0'), background: (act?.couleur || '#0ea888') + '10', color: act?.couleur || '#5a6480', fontFamily: 'inherit', fontSize: 12, fontWeight: 600 }}>
      ...
      {isFuture ? (
        ...
        <div style={S.card}>
          {(() => {
            ...
            return (
              <div style={{ padding: '10px 12px', background: '#d48f0010', border: '1px solid #d48f0033', borderRadius: 6, marginBottom: 10, fontSize: 11, color: '#633806' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <button onClick={() => {
                    ...
                    if (m) { openFiche(m.id) }
                  }} style={{ background: '#fff', border: '1px solid #d48f0055', borderRadius: 5, padding: '3px 8px', fontSize: 10, color: '#633806', cursor: 'pointer', fontFamily: 'inherit' }}>Ouvrir la fiche pour ajuster la date d'inscription</button>
                  ...
                  } })} style={{ background: '#fff', border: '1px solid #e0305055', borderRadius: 5, padding: '3px 8px', fontSize: 10, color: '#791F1F', cursor: 'pointer', fontFamily: 'inherit' }}>Supprimer ces présences</button>
                </div>
              </div>
            )
          })()}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
              <button onClick={() => { const o = {}; eligible.forEach(m => { o[m.id] = true }); setChk(o); setSaved(false) }} style={{ ...S.btn('#0ea888', true), padding: '5px 10px', fontSize: 11 }}>Tous</button>
              <button onClick={() => { setChk({}); setSaved(false) }} style={{ ...S.btn('#6b7280', true), padding: '5px 10px', fontSize: 11 }}>Aucun</button>
              {existing.length > 0 && <button onClick={handleDelete} style={{ ...S.btn('#e03050', true), display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', fontSize: 11 }}><Trash2 size={13} /> Suppr.</button>}
              {!(datesAnnulees || []).some(d => d.activite_id === actId && d.date_annulee === date) && (
                <button onClick={() => setConfirmAction({ msg: 'Annuler cette date ? Les absences ne seront pas comptabilisées.', input: true, fn: async (motif) => { try { await ajouterDateAnnulee(actId, date, motif || null) } catch(e) {} } })} style={{ ...S.btn('#d48f00', true), padding: '5px 10px', fontSize: 11 }}>Annuler date</button>
              )}
            </div>
          </div>
          ...
          <div style={{ maxHeight: 400, overflowY: 'auto' }}>
            {filtered.map(m => (
              <div key={m.id} onClick={() => { setChk(prev => ({ ...prev, [m.id]: !prev[m.id] })); setSaved(false) }}
                ...
                <div style={{ width: 22, height: 22, borderRadius: 5, border: '2px solid ' + (chk[m.id] ? '#1a9c60' : '#e0e4ec'), background: chk[m.id] ? '#1a9c60' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#fff', flexShrink: 0 }}>
                  {chk[m.id] ? '✓' : ''}
                </div>
                <span style={{ fontSize: 12, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.prenom} {m.nom}</span>
                <span style={{ fontSize: 10, color: '#6b7280', flexShrink: 0 }}>{m.role}</span>
              </div>
            ))}
