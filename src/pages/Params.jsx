...
function AccordionRefTable({ table, label, fields, showToast, familleId }) {
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      <div onClick={() => setOpen(!open)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: open ? '#B8733308' : '#FBF7F1', borderRadius: 7, cursor: 'pointer', border: '1px solid ' + (open ? '#B8733333' : '#EADFCF') }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: open ? '#B87333' : '#6B5D4A' }}>{label}</span>
        <span style={{ fontSize: 12, color: '#8B7355' }}>{open ? '▲' : '▼'}</span>
      </div>
...
function RefTable({ table, label, fields, showToast, familleId }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {rows.map(r => (
        <div key={r.id} style={{ padding: '6px 0', borderBottom: '1px solid #EADFCF', opacity: r.actif ? 1 : 0.4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {fields.includes('jour_semaine') && (
              ...
              }} style={{ fontSize: 11, padding: '3px 6px', border: '1px solid #EADFCF', borderRadius: 4, background: '#FBF7F1', fontFamily: 'inherit' }}>
            ...
            <button onClick={() => desactiver(r.id, !r.actif)} style={{ background: 'none', border: 'none', fontSize: 11, color: r.actif ? '#8B7355' : '#B87333', cursor: 'pointer' }}>
          ...
          {fields.includes('url') && (
            <div style={{ marginTop: 4, marginLeft: 20, display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              }} style={{ fontSize: 12, padding: '4px 8px', border: '1px solid #EADFCF', borderRadius: 4, background: '#FBF7F1', boxSizing: 'border-box', width: '100%' }} />
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                ...
                }} style={{ fontSize: 12, padding: '4px 8px', border: '1px solid #EADFCF', borderRadius: 4, background: '#FBF7F1', boxSizing: 'border-box', flex: 1 }} />
                {r.url && <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: '#B87333', flexShrink: 0 }}>Ouvrir</a>}
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
        ...
        <button onClick={handleAdd} style={S.btn('#B87333', false)}>+</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <button onClick={() => setShowInvite(!showInvite)} style={{ ...S.btn('#B87333', false), fontSize: 12, padding: '4px 10px' }}>{showInvite ? '✕ Fermer' : '+ Inviter'}</button>
      </div>

      {showInvite && (
        <div style={{ padding: 12, background: '#B8733308', border: '1px solid #B8733333', borderRadius: 8, marginBottom: 12 }}>
          <div style={{ fontSize: 12, color: '#6B5D4A', marginBottom: 8, lineHeight: 1.5 }}>
          ...
          <button onClick={handleInvite} disabled={inviting || !inviteData.email} style={{ ...S.btn('#B87333', false), width: '100%', opacity: inviting || !inviteData.email ? 0.6 : 1 }}>
      ...
      {profils.map(p => {
        ...
        return (
          <div key={p.id} style={{ padding: '8px 0', borderBottom: '1px solid #EADFCF' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: 120 }}>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{p.nom_affiche || p.email}</div>
                <div style={{ fontSize: 11, color: '#8B7355' }}>{p.email}</div>
              </div>
              ...
              {auth?.isSuperAdmin ? (
                <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, cursor: 'pointer', color: '#8B5B9E' }}>
                ...
                <span style={{ fontSize: 12, color: '#8B5B9E', fontWeight: 600 }} title="Attribué par le super-admin">Berger d'église</span>
              ) : null}
              {auth?.isSuperAdmin && (
                <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, cursor: 'pointer', color: '#C0563A' }} title="Accès technique total (bypass RLS). À n'accorder qu'en connaissance de cause.">
            ...
            <div style={{ marginTop: 4 }}>
              <select value={p.membre_id || ''} onChange={e => linkMembre(p.id, e.target.value || null)} style={{ fontSize: 11, padding: '3px 6px', border: '1px solid #EADFCF', borderRadius: 4, background: '#FBF7F1', color: '#6B5D4A', fontFamily: 'inherit', width: '100%', maxWidth: 250 }}>
              ...
              {linkedMembre && <span style={{ fontSize: 11, color: '#B87333', marginLeft: 6 }}>→ {linkedMembre.prenom} {linkedMembre.nom}</span>}
            </div>
            {familles.length > 0 && (
              <div style={{ marginTop: 4 }}>
                <select value={p.famille_id || ''} onChange={e => linkFamille(p.id, e.target.value || null)} style={{ fontSize: 11, padding: '3px 6px', border: '1px solid #EADFCF', borderRadius: 4, background: p.famille_id ? '#B8733308' : '#C0563A08', color: '#6B5D4A', fontFamily: 'inherit', width: '100%', maxWidth: 250 }}>
  ...
  const addFamille = async () => {
    ...
    try {
      ...
      const defaultActivites = [
        { nom: 'Culte du dimanche', code: 'culte', icone: '⛪', couleur: '#B87333', jour_semaine: 0, est_recurrente: true },
        { nom: 'Enseignement', code: 'enseignement', icone: '📖', couleur: '#B87333', jour_semaine: null, est_recurrente: false },
        { nom: 'Réunion de prière', code: 'priere', icone: '🙏', couleur: '#8B5B9E', jour_semaine: null, est_recurrente: false },
      ]
    ...
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        {eglises.map(e => {
          ...
          return (
            <div key={e.id} style={{ padding: '6px 0', borderBottom: '1px solid #EADFCF', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, opacity: e.actif === false ? 0.4 : 1 }}>
              <span style={{ fontWeight: 600, flex: 1 }}>{e.nom}{e.actif === false && <span style={{ fontSize: 10, marginLeft: 6, color: '#8B7355' }}>(désactivée)</span>}</span>
              <span style={{ fontSize: 11, color: '#8B7355' }}>{nbFamActives}/{nbFam} famille(s)</span>
              ...
              }} style={{ background: 'none', border: 'none', fontSize: 11, color: e.actif !== false ? '#8B7355' : '#B87333', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
          ...
          <button onClick={addEglise} style={S.btn('#B87333', false)}>+</button>
        </div>
      </div>
      ...
      <div>
        ...
        {familles.map(f => {
          ...
          return (
            <div key={f.id} style={{ padding: '6px 0', borderBottom: '1px solid #EADFCF', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, opacity: f.actif === false ? 0.4 : 1 }}>
              <div style={{ flex: 1 }}>
                <span style={{ fontWeight: 600 }}>{f.nom}</span>
                <span style={{ fontSize: 11, color: '#8B7355', marginLeft: 6 }}>({eg?.nom}){f.actif === false && ' (désactivée)'}</span>
              </div>
              <button onClick={async () => {
                try {
              ...
              }} style={{ background: 'none', border: 'none', fontSize: 11, color: f.actif !== false ? '#8B7355' : '#B87333', cursor: 'pointer' }}>
        ...
        {eglises.length > 0 && (
          <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
            ...
            <button onClick={addFamille} style={S.btn('#B87333', false)}>+</button>
          </div>
        )}
      </div>

      {eglises.length === 0 && (
        <div style={{ padding: 12, textAlign: 'center', color: '#8B7355', fontSize: 12 }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        <button onClick={() => setTab('refs')} style={{ ...S.btn(tab === 'refs' ? '#B87333' : '#8B7355', tab !== 'refs'), fontFamily: 'inherit' }}>Références</button>
        <button onClick={() => setTab('users')} style={{ ...S.btn(tab === 'users' ? '#B87333' : '#8B7355', tab !== 'users'), fontFamily: 'inherit' }}>Utilisateurs</button>
        <button onClick={() => setTab('eglise')} style={{ ...S.btn(tab === 'eglise' ? '#B87333' : '#8B7355', tab !== 'eglise'), fontFamily: 'inherit' }}>Église</button>
      </div>
      <div style={S.card}>
        {tab === 'users' ? <UsersTable showToast={showToast} actifs={actifs} refs={refs} auth={auth} />
          ...
          : <>
            ...
            {activitesTable && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px dashed #EADFCF' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#B87333' }}>Activités</span>
                  <select value={selectedFamilleId} onChange={e => setSelectedFamilleId(e.target.value)} style={{ flex: 1, padding: '5px 8px', borderRadius: 6, border: '1px solid #DCC9B0', background: '#FBF7F1', fontSize: 12 }}>
                    {familles.map(f => <option key={f.id} value={f.id}>{f.nom}{f.eglises?.nom ? ' (' + f.eglises.nom + ')' : ''}</option>)}
                  </select>
                </div>
                ...
                {!selectedFamilleId && <div style={{ fontSize: 12, color: '#8B7355', padding: 8 }}>Sélectionnez une famille pour gérer ses activités.</div>}
              </div>
            )}
          </>}
      </div>
