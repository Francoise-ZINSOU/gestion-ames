...
export default function PresencesPage({ actifs, presences, refs, h, enregistrerPresences, supprimerDate, auth, datesAnnulees, ajouterDateAnnulee, supprimerDateAnnulee, showToast, openFiche }) {
  ...
  if (!activites.length) return (
    <div style={{ ...S.card, textAlign: 'center', color: '#5E7175', fontSize: 14, lineHeight: 1.7, padding: 30 }}>
    ...
    <div>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <div className="desk-only" style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          {activites.map(a => (
            <button key={a.id} onClick={() => setActId(a.id)} style={{
              ...
              border: '1px solid ' + (actId === a.id ? a.couleur : '#DCE6E5'),
              background: actId === a.id ? a.couleur + '12' : 'transparent',
              color: actId === a.id ? a.couleur : '#5E7175', fontWeight: actId === a.id ? 600 : 500,
              whiteSpace: 'nowrap'
        ...
        <input type="date" value={date} max={today()} onChange={e => setDate(e.target.value)}
          style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid #C3D4D3', background: '#F5F3EE', fontSize: 12, fontFamily: 'inherit', marginLeft: 'auto' }} />
      </div>
      ...
      {(() => {
        ...
        if (cancelled) return (
          <div style={{ padding: '12px 16px', borderRadius: 8, background: '#2E7D8A10', border: '1px solid #2E7D8A33', fontSize: 13, color: '#2E7D8A', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>Cette date a été annulée{cancelled.motif ? ' : ' + cancelled.motif : ''}. Les absences ne comptent pas.</span>
            <button onClick={async () => { await supprimerDateAnnulee(cancelled.id) }} style={{ background: 'none', border: '1px solid #2E7D8A', borderRadius: 5, padding: '3px 10px', fontSize: 12, color: '#2E7D8A', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', marginLeft: 8 }}>Rétablir</button>
          </div>
        )
        return null
      })()}
      ...
      {isFuture ? (
        <div style={{ padding: '12px 16px', borderRadius: 8, background: '#C25A4A08', border: '1px solid #C25A4A33', fontSize: 13, color: '#C25A4A' }}>
        ...
        <div style={S.card}>
          {(() => {
            ...
            return (
              <div style={{ padding: '10px 12px', background: '#2E7D8A10', border: '1px solid #2E7D8A33', borderRadius: 6, marginBottom: 10, fontSize: 12, color: '#633806' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: '#5E7175', marginTop: 2 }}>
          ...
          {existing.length > 0 && existing[0]?.created_by && auth?.session?.user?.id && existing[0].created_by !== auth.session.user.id && (
            <div style={{ padding: '6px 10px', background: '#FAEEDA', borderRadius: 6, marginBottom: 10, fontSize: 12, color: '#633806', borderLeft: '3px solid #2E7D8A' }}>
          ...
          <div style={{ maxHeight: 400, overflowY: 'auto' }}>
            {filtered.map(m => (
              <div key={m.id} onClick={() => { setChk(prev => ({ ...prev, [m.id]: !prev[m.id] })); setSaved(false) }}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 8px', borderBottom: '1px solid #DCE6E5', cursor: 'pointer', background: chk[m.id] ? '#4E8D6E08' : 'transparent', minHeight: 44 }}>
                <div style={{ width: 22, height: 22, borderRadius: 5, border: '2px solid ' + (chk[m.id] ? '#4E8D6E' : '#DCE6E5'), background: chk[m.id] ? '#4E8D6E' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: '#fff', flexShrink: 0 }}>
                  {chk[m.id] ? '✓' : ''}
                </div>
                <span style={{ fontSize: 13, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.prenom} {m.nom}</span>
                <span style={{ fontSize: 11, color: '#5E7175', flexShrink: 0 }}>{m.role}</span>
              </div>
            ))}
            {filtered.length === 0 && <div style={{ padding: 14, textAlign: 'center', color: '#5E7175', fontSize: 13 }}>Aucun membre éligible</div>}
          </div>

          {/* Barre sticky en bas — Enregistrer */}
          <div className="sticky-save" style={{ position: 'sticky', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #DCE6E5', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10, boxShadow: '0 -2px 8px rgba(0,0,0,.06)', borderRadius: '0 0 12px 12px' }}>
            <div style={{ fontSize: 13, color: '#5E7175' }}>
              <strong style={{ fontSize: 16, color: '#2B3A3D' }}>{nChecked}</strong> / {eligible.length}
            </div>
            <button onClick={handleSave} style={{ ...S.btn(saved ? '#5E7175' : '#4E8D6E', false), display: 'flex', alignItems: 'center', gap: 5, padding: '10px 20px', fontSize: 14 }}>
        ...
        <div className="modal-overlay danger">
          <div className="modal-box" style={{ maxWidth: 380 }}>
            <div style={{ padding: '20px 24px' }}>
              ...
              <div style={{ fontSize: 14, color: '#5E7175', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{confirmAction.msg}</div>
              {confirmAction.input && (
            ...
            <div style={{ padding: '12px 24px', borderTop: '1px solid #DCE6E5', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => { setConfirmAction(null); setConfirmInput('') }} style={S.btn('#5E7175', true)}>Annuler</button>
              <button onClick={() => { confirmAction.fn(confirmInput); setConfirmAction(null); setConfirmInput('') }} style={S.btn('#C25A4A', false)}>Confirmer</button>
            </div>
          </div>
        </div>
      )}
