...
export default function EntretiensPage({ entretiens, membres, actifs, refs, h, openFiche, ajouterEnt, showToast }) {
  ...
  const stColor = (s) => (refs.statutsEntretien || []).find(x => x.nom === s)?.couleur || '#8B7355'
  ...
  return (
    <div>
      {planifiesRetard.length > 0 && (
        <div style={{ ...S.card, borderLeft: '3px solid #B87333', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: '#B87333', marginBottom: 6 }}><AlertTriangle size={14} /> {planifiesRetard.length} entretien(s) planifié(s) en retard</div>
          {planifiesRetard.map(e => (
            <div key={e.id} onClick={() => openFiche(e.membre_id)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '3px 0', borderBottom: '1px solid #EADFCF', cursor: 'pointer' }}>
              <span style={{ color: '#C0563A', width: 65, flexShrink: 0 }}>{fmtS(e.date_entretien)}</span>
              <span style={{ fontWeight: 600, color: '#B87333' }}>{e.mn}</span>
              <span style={{ color: '#6B5D4A', flex: 1 }}>{e.sujet}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={S.card}>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10, alignItems: 'center' }}>
          <input type="date" value={fDateDe} onChange={e => setFDateDe(e.target.value)} title="Du" style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid #DCC9B0', background: '#FBF7F1', fontSize: 12, boxSizing: 'border-box' }} />
          <input type="date" value={fDateA} onChange={e => setFDateA(e.target.value)} title="Au" style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid #DCC9B0', background: '#FBF7F1', fontSize: 12, boxSizing: 'border-box' }} />
          <select value={fAvec} onChange={e => setFAvec(e.target.value)} style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid #DCC9B0', background: '#FBF7F1', fontSize: 12 }}>
          ...
          <select value={fStatut} onChange={e => setFStatut(e.target.value)} style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid #DCC9B0', background: '#FBF7F1', fontSize: 12 }}>
            <option value="">Tous statuts</option>
            {(refs.statutsEntretien || []).map(s => <option key={s.nom} value={s.nom}>{s.nom}</option>)}
          </select>
          {(fDateDe || fDateA || fAvec || fStatut) && <button onClick={() => { setFDateDe(''); setFDateA(''); setFAvec(''); setFStatut('') }} style={{ background: 'none', border: 'none', fontSize: 11, color: '#C0563A', cursor: 'pointer' }}>✕ Effacer</button>}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div style={{ fontSize: 14, fontWeight: 600 }}>Entretiens ({all.length})</div>
          <button onClick={() => { setFd({ statut: h.defaultStatutEnt, date_entretien: today() }); setModal('new') }} style={{ ...S.btn('#B87333', false), display: 'flex', alignItems: 'center', gap: 4 }}><Plus size={13} /> Entretien</button>
        </div>
        {all.length === 0 ? <div style={{ padding: 12, textAlign: 'center', color: '#8B7355', fontSize: 13 }}>Aucun entretien enregistré. <span onClick={() => { setFd({ statut: h.defaultStatutEnt, date_entretien: today() }); setModal('new') }} style={{ color: '#B87333', cursor: 'pointer', textDecoration: 'underline' }}>Créer le premier</span></div>
          : <div>
            {/* Desktop */}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>{all.slice(0, showCount).map(e => (
                  <tr key={e.id} onClick={() => openFiche(e.membre_id)} style={{ cursor: 'pointer' }}>
                    <td style={{ ...S.td, color: '#6B5D4A' }}>{fmtS(e.date_entretien)}</td>
                    <td style={{ ...S.td, fontWeight: 600, color: '#B87333' }}>{e.mn}</td>
                    <td style={{ ...S.td, color: '#6B5D4A' }}>{e.avecNom}</td>
            ...
            <div className="mob-only">
              {all.slice(0, showCount).map(e => (
                <div key={e.id} onClick={() => openFiche(e.membre_id)} style={{ padding: '10px 6px', borderBottom: '1px solid #EADFCF', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: '#B87333' }}>{e.mn}</span>
                    <span style={S.pill(stColor(e.statut))}>{e.statut}</span>
                  </div>
                  <div style={{ fontSize: 12, color: '#8B7355' }}>{fmtS(e.date_entretien)} · {e.avecNom}{e.sujet ? ' · ' + e.sujet : ''}</div>
                </div>
              ))}
            </div>
            {all.length > showCount && (
              <div onClick={() => setShowCount(prev => prev + 30)} style={{ padding: '10px 0', textAlign: 'center', fontSize: 13, color: '#B87333', fontWeight: 600, cursor: 'pointer' }}>
      ...
      {modal === 'new' && (
        <div className="modal-overlay">
          <div className="modal-box" style={{ maxWidth: 460 }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #EADFCF' }}><div style={{ fontSize: 15, fontWeight: 700 }}>Nouvel entretien</div></div>
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid #EADFCF', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleSave} style={S.btn('#B87333', false)}>Enregistrer</button>
            </div>
          </div>
        </div>
      )}
