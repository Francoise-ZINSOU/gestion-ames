  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', minHeight: '100dvh', background: '#FBF7F1', fontFamily: 'DM Sans, sans-serif', padding: '60px 16px' }}>
      <div style={{ width: '100%', maxWidth: 420, background: '#fff', borderRadius: 16, overflow: 'hidden', textAlign: 'center', padding: '40px 28px', boxShadow: '0 4px 24px rgba(0,0,0,.06)' }}>
        ...
        <div style={{ fontSize: 14, color: '#6B5D4A', lineHeight: 1.7, marginBottom: 20 }}>
          Bienvenue <strong>{profil?.nom_affiche || profil?.email}</strong> !<br />
          Votre compte existe mais n'a pas encore été activé.
        </div>

        <div style={{ textAlign: 'left', padding: '14px 16px', background: '#B8733308', border: '1px solid #B8733333', borderRadius: 10, marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#B87333', marginBottom: 8 }}>Comment obtenir l'accès ?</div>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
          <button onClick={() => window.location.reload()} style={S.btn('#B87333', false)}>Rafraîchir</button>
          <button onClick={logout} style={S.btn('#8B7355', true)}>Se déconnecter</button>
        </div>
      </div>
    </div>
  )
