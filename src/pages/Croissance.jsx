...
export default function CroissancePage({ plans, refs }) {
  ...
  return (
    <div style={S.card}>
      <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Avancement des plans de croissance</div>
      {stats.map(s => (
        <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          <span style={{ fontSize: 11, color: '#5a6480', width: 180, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.nom}</span>
          ...
          <span style={{ fontSize: 10, fontWeight: 600, width: 42, textAlign: 'right' }}>{s.vn}/{s.an}</span>
        </div>
      ))}
    </div>
  )
