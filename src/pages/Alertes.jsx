...
export default function AlertesPage({ alertes, membres, openFiche, entretiens, refs }) {
  ...
  return (
    <div style={S.card}>
      ...
      <div style={{ fontSize: 12, color: '#64748B', marginBottom: 10 }}>Plus le score est élevé, plus le membre a besoin d'attention. Cliquez sur un membre pour ouvrir sa fiche.. Les membres avec un entretien planifié dans les 30 prochains jours sont masqués.</div>
      {filtered.length === 0 ? <div style={{ padding: 12, textAlign: 'center', color: '#059669', fontSize: 13 }}>✓ Aucune alerte — tout va bien !</div>
        : filtered.map(a => {
          ...
          const details = []
          if (a.absences >= 3) details.push({ label: a.absences + ' abs. consécutives', pts: 3, color: '#E11D48' })
          if (!hasPlanned && a.jours_sans_entretien > 21) details.push({ label: (a.jours_sans_entretien < 30 ? a.jours_sans_entretien + 'j' : a.jours_sans_entretien < 365 ? Math.floor(a.jours_sans_entretien / 30) + ' mois' : Math.floor(a.jours_sans_entretien / 365) + ' an(s)') + ' sans entretien', pts: 2, color: '#BA7517' })
          if (a.defis_ouverts > 0) details.push({ label: a.defis_ouverts + ' défi(s) ouvert(s)', pts: 1, color: '#7040d0' })
          return (
            <div key={a.membre_id} onClick={() => openFiche(a.membre_id)} style={{ padding: '10px 8px', borderBottom: '1px solid #E2E8F0', cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: a.score_severite >= 4 ? '#E11D481a' : '#BA75171a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: a.score_severite >= 4 ? '#E11D48' : '#BA7517', fontWeight: 700, flexShrink: 0 }}>{a.score_severite}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#185FA5' }}>{a.nom_complet}</div>
                  <div style={{ fontSize: 11, color: '#64748B' }}>suivi par {getSuiveur(a.suivi_par)}</div>
                </div>
              </div>
