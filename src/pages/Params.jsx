...
function AccordionRefTable({ table, label, fields, showToast, familleId }) {
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      <div onClick={() => setOpen(!open)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: open ? '#2E7D8A08' : '#F5F3EE', borderRadius: 7, cursor: 'pointer', border: '1px solid ' + (open ? '#2E7D8A33' : '#DCE6E5') }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: open ? '#2E7D8A' : '#5E7175' }}>{label}</span>
        <span style={{ fontSize: 12, color: '#5E7175' }}>{open ? '▲' : '▼'}</span>
      </div>
...
function RefTable({ table, label, fields, showToast, familleId }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {rows.map(r => (
        <div key={r.id} style={{ padding: '6px 0', borderBottom: '1px solid #DCE6E5', opacity: r.actif ? 1 : 0.4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {fields.includes('jour_semaine') && (
              ...
              }} style={{ fontSize: 11, padding: '3px 6px', border: '1px solid #DCE6E5', borderRadius: 4, background: '#F5F3EE', fontFamily: 'inherit' }}>
          ...
          {fields.includes('url') && (
            <div style={{ marginTop: 4, marginLeft: 20, display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              }} style={{ fontSize: 12, padding: '4px 8px', border: '1px solid #DCE6E5', borderRadius: 4, background: '#F5F3EE', boxSizing: 'border-box', width: '100%' }} />
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                ...
                }} style={{ fontSize: 12, padding: '4px 8px', border: '1px solid #DCE6E5', borderRadius: 4, background: '#F5F3EE', boxSizing: 'border-box', flex: 1 }} />
                {r.url && <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: '#2E7D8A', flexShrink: 0 }}>Ouvrir</a>}
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
        ...
        <button onClick={handleAdd} style={S.btn('#2E7D8A', false)}>+</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <button onClick={() => setShowInvite(!showInvite)} style={{ ...S.btn('#2E7D8A', false), fontSize: 12, padding: '4px 10px' }}>{showInvite ? '✕ Fermer' : '+ Inviter'}</button>
      </div>

      {showInvite && (
        <div style={{ padding: 12, background: '#2E7D8A08', border: '1px solid #2E7D8A33', borderRadius: 8, marginBottom: 12 }}>
          <div style={{ fontSize: 12, color: '#5E7175', marginBottom: 8, lineHeight: 1.5 }}>
      ...
      {profils.map(p => {
        ...
        return (
          <div key={p.id} style={{ padding: '8px 0', borderBottom: '1px solid #DCE6E5' }}>
            ...
            <div style={{ marginTop: 4 }}>
              <select value={p.membre_id || ''} onChange={e => linkMembre(p.id, e.target.value || null)} style={{ fontSize: 11, padding: '3px 6px', border: '1px solid #DCE6E5', borderRadius: 4, background: '#F5F3EE', color: '#5E7175', fontFamily: 'inherit', width: '100%', maxWidth: 250 }}>
  ...
  const addFamille = async () => {
    ...
    try {
      ...
      const defaultActivites = [
        { nom: 'Culte du dimanche', code: 'culte', icone: '⛪', couleur: '#2E7D8A', jour_semaine: 0, est_recurrente: true },
        { nom: 'Enseignement', code: 'enseignement', icone: '📖', couleur: '#2E7D8A', jour_semaine: null, est_recurrente: false },
    ...
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        {eglises.map(e => {
          ...
          return (
            <div key={e.id} style={{ padding: '6px 0', borderBottom: '1px solid #DCE6E5', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, opacity: e.actif === false ? 0.4 : 1 }}>
              <span style={{ fontWeight: 600, flex: 1 }}>{e.nom}{e.actif === false && <span style={{ fontSize: 10, marginLeft: 6, color: '#5E7175' }}>(désactivée)</span>}</span>
              <span style={{ fontSize: 11, color: '#5E7175' }}>{nbFamActives}/{nbFam} famille(s)</span>
              ...
              }} style={{ background: 'none', border: 'none', fontSize: 11, color: e.actif !== false ? '#5E7175' : '#2E7D8A', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
          ...
          <button onClick={addEglise} style={S.btn('#2E7D8A', false)}>+</button>
        </div>
      </div>
      ...
      <div>
        ...
        {familles.map(f => {
          ...
          return (
            <div key={f.id} style={{ padding: '6px 0', borderBottom: '1px solid #DCE6E5', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, opacity: f.actif === false ? 0.4 : 1 }}>
              <div style={{ flex: 1 }}>
                <span style={{ fontWeight: 600 }}>{f.nom}</span>
                <span style={{ fontSize: 11, color: '#5E7175', marginLeft: 6 }}>({eg?.nom}){f.actif === false && ' (désactivée)'}</span>
              </div>
              <button onClick={async () => {
                try {
              ...
              }} style={{ background: 'none', border: 'none', fontSize: 11, color: f.actif !== false ? '#5E7175' : '#2E7D8A', cursor: 'pointer' }}>
        ...
        {eglises.length > 0 && (
          <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
            ...
            <button onClick={addFamille} style={S.btn('#2E7D8A', false)}>+</button>
          </div>
        )}
      </div>

      {eglises.length === 0 && (
        <div style={{ padding: 12, textAlign: 'center', color: '#5E7175', fontSize: 12 }}>
      ...
      <div style={S.card}>
        {tab === 'users' ? <UsersTable showToast={showToast} actifs={actifs} refs={refs} auth={auth} />
          ...
          : <>
            ...
            {activitesTable && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px dashed #DCE6E5' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#2E7D8A' }}>Activités</span>
                  <select value={selectedFamilleId} onChange={e => setSelectedFamilleId(e.target.value)} style={{ flex: 1, padding: '5px 8px', borderRadius: 6, border: '1px solid #C3D4D3', background: '#F5F3EE', fontSize: 12 }}>
                    {familles.map(f => <option key={f.id} value={f.id}>{f.nom}{f.eglises?.nom ? ' (' + f.eglises.nom + ')' : ''}</option>)}
                  </select>
                </div>
                ...
                {!selectedFamilleId && <div style={{ fontSize: 12, color: '#5E7175', padding: 8 }}>Sélectionnez une famille pour gérer ses activités.</div>}
              </div>
            )}
          </>}
      </div>
