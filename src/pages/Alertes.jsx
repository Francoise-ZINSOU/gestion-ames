    ...
    <div style={S.card}>
      ...
      {filtered.length === 0 ? <div style={{ padding: 12, textAlign: 'center', color: '#4E8D6E', fontSize: 13 }}>✓ Aucune alerte — tout va bien !</div>
        : filtered.map(a => {
          ...
          if (!hasPlanned && a.jours_sans_entretien >= 30) details.push({ label: (a.jours_sans_entretien < 30 ? a.jours_sans_entretien + 'j' : a.jours_sans_entretien < 365 ? Math.floor(a.jours_sans_entretien / 30) + ' mois' : Math.floor(a.jours_sans_entretien / 365) + ' an(s)') + ' sans entretien', pts: 2, color: '#2E7D8A' })
