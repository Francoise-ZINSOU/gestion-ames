...
export default function ExportPage({ membres, presences, entretiens, defis, refs, showToast }) {
  ...
  const doExport = (type) => {
    ...
    if (type === 'membres') {
      ...
      rows = entretiens.map(e => { const m = membres.find(x => x.id === e.membre_id); const sujet = e.sujet_libre || (refs.sujetsEntretien || []).find(s => s.id === e.sujet_id)?.nom || ''; return [e.membre_id, m?.prenom, m?.nom, e.date_entretien, getSuiveur(e.avec_qui), sujet, e.statut, e.commentaires].map(esc).join(',') })
    } else if (type === 'defis') {
  ...
  const doReset = async () => {
    setResetting(true)
    try { await resetAllData(); showToast('✓ Données supprimées'); window.location.reload() }
