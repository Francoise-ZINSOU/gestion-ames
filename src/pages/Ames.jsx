  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', marginBottom: 8 }}>
        <Search size={14} style={{ position: 'absolute', left: 10, top: 9, color: '#5E7175' }} />
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
        <select value={fRole} onChange={e => setFRole(e.target.value)} style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid #C3D4D3', background: '#F5F3EE', fontFamily: 'inherit', fontSize: 12, color: '#2B3A3D', outline: 'none' }}>
          <option value="all">Tous rôles</option>
          {(refs.roles || []).map(r => <option key={r.nom} value={r.nom}>{r.nom}</option>)}
        </select>
        <select value={fSt} onChange={e => setFSt(e.target.value)} style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid #C3D4D3', background: '#F5F3EE', fontFamily: 'inherit', fontSize: 12, color: '#2B3A3D', outline: 'none' }}>
      ...
      {bulkMode && (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 8, padding: '6px 10px', background: '#8B5B9E08', borderRadius: 7, border: '1px solid #8B5B9E33', flexWrap: 'wrap' }}>
          ...
          {Object.values(bulkSel).filter(Boolean).length > 0 && (
            <>
              <select id="bulk-statut" style={{ fontSize: 12, padding: '4px 6px', borderRadius: 4, border: '1px solid #DCE6E5' }}>
              ...
              <select id="bulk-suiveur" style={{ fontSize: 12, padding: '4px 6px', borderRadius: 4, border: '1px solid #DCE6E5' }}>
              ...
              }} style={{ ...S.btn('#2E7D8A', false), padding: '4px 8px', fontSize: 12 }}>OK</button>
            </>
          )}
          <button onClick={() => { setBulkMode(false); setBulkSel({}) }} style={{ ...S.btn('#5E7175', true), padding: '4px 8px', fontSize: 12, marginLeft: 'auto' }}>✕</button>
        </div>
      )}
          ...
          <div>
                ...
                <tbody>{filt.map(m => {
                  ...
                  return (
                    <tr key={m.id} onClick={() => bulkMode ? setBulkSel(prev => ({ ...prev, [m.id]: !prev[m.id] })) : openFiche(m.id)} style={{ cursor: 'pointer', opacity: m.archive ? 0.5 : 1, background: bulkSel[m.id] ? '#8B5B9E08' : 'transparent' }}>
                      ...
                      <td style={S.td}><span style={{ fontWeight: 600, color: '#2E7D8A' }}>{m.prenom} {m.nom}</span></td>
                      <td style={S.td}><span style={S.pill(getRoleColor(refs, m.role))}>{m.role}</span></td>
                      <td style={{ ...S.td, color: '#5E7175' }}>{fmtS(m.date_inscription)}</td>
                      <td style={{ ...S.td, color: '#5E7175' }}>{getSuiveur(m.suivi_par)}</td>
                      <td style={S.td}><span style={S.pill(getStatutColor(refs, m.statut))}>{m.statut}</span></td>
                      <td style={{ ...S.td, fontWeight: 600, color: t !== null ? (t >= 80 ? '#4E8D6E' : t >= 50 ? '#2E7D8A' : '#C25A4A') : '#5E7175' }}>{t !== null ? t + '%' : '—'}</td>
                      <td style={{ ...S.td, fontWeight: 600, color: mEn(m.id) ? '#2E7D8A' : '#5E7175' }}>{mEn(m.id)}</td>
                    </tr>
                  )
                })}</tbody>
              </table>
            ...
            <div className="mob-only">
              {filt.map(m => {
                const t = taux(m.id)
                return (
                  <div key={m.id} onClick={() => bulkMode ? setBulkSel(prev => ({ ...prev, [m.id]: !prev[m.id] })) : openFiche(m.id)} style={{ padding: '10px 8px', borderBottom: '1px solid #DCE6E5', cursor: 'pointer', opacity: m.archive ? 0.5 : 1, background: bulkSel[m.id] ? '#8B5B9E08' : 'transparent' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                      {bulkMode && <input type="checkbox" checked={!!bulkSel[m.id]} readOnly />}
                      <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: '#2E7D8A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.prenom} {m.nom}</span>
                      <span style={S.pill(getStatutColor(refs, m.statut))}>{m.statut}</span>
                    </div>
                    <div style={{ display: 'flex', gap: 8, fontSize: 11, color: '#5E7175', alignItems: 'center', flexWrap: 'wrap' }}>
                      <span style={{ ...S.pill(getRoleColor(refs, m.role)), fontSize: 10, padding: '1px 6px' }}>{m.role}</span>
                      {t !== null && <span style={{ fontWeight: 600, color: t >= 80 ? '#4E8D6E' : t >= 50 ? '#2E7D8A' : '#C25A4A' }}>{t}%</span>}
                      {mEn(m.id) > 0 && <span style={{ color: '#2E7D8A' }}>{mEn(m.id)} ent.</span>}
          ...
          <div className="modal-box" style={{ maxWidth: 500 }}>
            ...
            <div style={{ padding: '16px 20px', maxHeight: '55dvh', overflowY: 'auto' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 8px' }}>
                <div style={{ marginBottom: 8 }}><label style={S.label}>Nom</label><input value={fd.nom || ''} onChange={e => uf('nom', e.target.value)} style={{ ...S.inp, borderColor: fd.nom === '' ? '#C25A4A' : '#C3D4D3' }} /></div>
                <div style={{ marginBottom: 8 }}><label style={S.label}>Prénom</label><input value={fd.prenom || ''} onChange={e => uf('prenom', e.target.value)} style={{ ...S.inp, borderColor: fd.prenom === '' ? '#C25A4A' : '#C3D4D3' }} /></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 8px' }}>
                <div style={{ marginBottom: 8 }}><label style={S.label}>Téléphone</label><input value={fd.telephone || ''} onChange={e => uf('telephone', e.target.value)} style={{ ...S.inp, borderColor: fd.telephone && !validTel(fd.telephone) ? '#C25A4A' : '#C3D4D3' }} placeholder="+225 07..." /></div>
                <div style={{ marginBottom: 8 }}><label style={S.label}>Email</label><input value={fd.email || ''} onChange={e => uf('email', e.target.value)} style={{ ...S.inp, borderColor: fd.email && !validEmail(fd.email) ? '#C25A4A' : '#C3D4D3' }} type="email" /></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 8px' }}>
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid #DCE6E5', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ ...S.btn('#2E7D8A', false), opacity: saving ? 0.6 : 1 }}>{saving ? 'Enregistrement...' : 'Enregistrer'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === 'reassign' && fd._reassignSuivis && (
        <div className="modal-overlay">
          <div className="modal-box" style={{ maxWidth: 460 }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #DCE6E5' }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#2E7D8A' }}>Réassigner avant changement de rôle</div>
            </div>
            <div style={{ padding: '16px 20px' }}>
              <div style={{ fontSize: 13, color: '#5E7175', marginBottom: 10 }}>Ce membre suit {fd._reassignSuivis.length} personne(s). Choisissez un nouveau responsable :</div>
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid #DCE6E5', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => { setModal(null); setFd({}) }} style={S.btn('#5E7175', true)}>Annuler</button>
              <button onClick={doReassign} style={S.btn('#2E7D8A', false)}>Réassigner et modifier</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === 'import' && (
        <div className="modal-overlay">
          <div className="modal-box" style={{ maxWidth: 520 }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #DCE6E5' }}><div style={{ fontSize: 15, fontWeight: 700 }}>Importer des membres (CSV)</div></div>
            <div style={{ padding: '16px 20px' }}>
              {!fd._csvRows ? (
                <div>
                  <div style={{ fontSize: 13, color: '#5E7175', marginBottom: 10, lineHeight: 1.6 }}>Préparez un fichier CSV avec les colonnes : Prénom, Nom, Téléphone, Email, Date inscription</div>
                  <div style={{ padding: '8px 12px', background: '#F5F3EE', borderRadius: 6, fontSize: 12, fontFamily: "'Roboto Mono', monospace", marginBottom: 12, lineHeight: 1.8 }}>
                    Prénom,Nom,Téléphone,Email<br/>Marina,N'GUESSAN,+225 07 12 34 56,marina@email.com<br/>David,Mensah,+225 05 33 44 55,
                  </div>
                  <div style={{ fontSize: 12, color: '#5E7175', marginBottom: 10 }}>Seuls Prénom et Nom sont obligatoires. Statut = Nouveau, Rôle = Membre par défaut.</div>
                ...
                <div>
                  <div style={{ fontSize: 13, color: '#5E7175', marginBottom: 8 }}>
                    {fd._csvRows.length} ligne(s). {fd._csvRows.filter(r => r._dup).length > 0 && <span style={{ color: '#2E7D8A' }}>⚠ {fd._csvRows.filter(r => r._dup).length} doublon(s) en jaune.</span>}
                  </div>
                  <div style={{ maxHeight: '30dvh', overflowY: 'auto' }}>
                    {fd._csvRows.map((r, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 6px', borderBottom: '1px solid #DCE6E5', background: r._dup ? '#FAEEDA' : 'transparent', opacity: r._skip ? 0.4 : 1 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 11, color: '#5E7175' }}>{[r.telephone, r.email].filter(Boolean).join(' · ')}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {fd._csvRows && (() => {
                ...
                return toImport.length > 0 && (
                  <button disabled={saving} onClick={async () => {
                    ...
                    setSaving(false)
                  }} style={{ ...S.btn('#2E7D8A', false), opacity: saving ? 0.6 : 1 }}>
